...
/**
 * Navigate to the node view where XPU/GPU panel is automatically visible.
 * In PVE 9, our plugin adds the XPU panel to the node view via override,
 * so it's visible as soon as you navigate to the node.
 */
export async function openXpuTab(page: Page) {
    await navigateToNode(page);

    // Click the XPU/GPU item in the tree navigation
    const xpuItem = page.locator('.x-treelist-item-text:has-text("XPU/GPU")').first();
    await xpuItem.scrollIntoViewIfNeeded();
    await xpuItem.click({ timeout: 10000 });
    await page.waitForTimeout(2000);
}
